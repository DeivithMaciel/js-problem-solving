...
function alfabeticoInverso() {
    usuarios.sort((a, b) => b.name.localeCompare(a.name))
}

function alternacaoAlfabetica() {
    if (usuarios.length === 0) {
        alert('Carregar lista primeiro')
        return
    }
    if (!alfabetizado) {
        alfabetico()
        alfabetizado = true
        alfabetica.innerHTML = "A-Z"
        renderUsers(usuarios)
    } else {
        alfabeticoInverso()
        alfabetizado = false
        alfabetica.innerHTML = 'Z-A'
        renderUsers(usuarios)
    }
}
...
alfabetica.addEventListener('click', alternacaoAlfabetica);
